...
export function ToolDrawer({
  ...
  return (
    <div
      className={`fixed bottom-0 left-1/2 -translate-x-1/2 h-max max-h-104 w-max bg-neutral-700 shadow-2xl rounded-t-lg transform transition-transform duration-300 ease-in-out z-40 ${
        toolDrawerOpen ? "translate-y-0" : "translate-y-[calc(100%-3rem)]"
      }`}
    >
      <div
        onClick={() => setToolDrawerOpen(!toolDrawerOpen)}
        className="px-6 py-3 rounded-t-lg cursor-pointer hover:bg-neutral-600 transition-colors flex items-center justify-center gap-3 shadow-lg"
      >
